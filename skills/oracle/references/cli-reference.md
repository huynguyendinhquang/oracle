...
- ⚠️ **Browser model-label drift:** the live ChatGPT picker is one compact menu `Instant / Medium / High / Extra High / Pro`. Pick tier via `-m gpt-5.5 --browser-thinking-time medium|high|extra-high` (Thinking tiers) or `-m gpt-5.5-pro --browser-thinking-time standard|extended` (Pro). `gpt-5.2-instant` by version-name fails → Instant only via `--engine api` (browser Instant select silently no-ops to the active tier — it's a top-level compact-menu tier, not a thinking-effort; fix in progress, re-verify after upgrades). Gemini browser works (`-m gemini-3.1-pro`, header/RPC select). Grok = API-only (no browser).
